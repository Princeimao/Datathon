...
function extractRegistration(text = "") {
  return (
    text
      .match(/\b[A-Z]{2}\s?\d{1,2}\s?[A-Z]{1,3}\s?\d{3,4}\b/i)?.[0]
      ?.replace(/\s+/g, "")
      .toUpperCase() || null
  );
}
...
export async function analyzeMediaFiles(
  ...
  for (const file of files) {
    const stored = await uploadToStratus({
      keyPrefix: isVideo(file.contentType)
        ? "evidence/videos"
        : "evidence/images",
    ...
    const detailText = [
      ...
      .filter(Boolean)
      .join(" ");
    const entityKind = isVideo(file.contentType)
      ? "VIDEO"
    ...
    intelligence.evidence.push({
      ...
      description:
        file.details ||
        file.label ||
        `${entityKind} evidence stored in Stratus`,
      confidence: 0.72,
      fileUrl: stored.url,
      extractedData: {
