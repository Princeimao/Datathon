...
function isPersonRelationshipType(
  type: string,
): type is
  | "FAMILY"
  | "ACCOMPLICE"
...
function stringToCoords(
  ...
  const lat = 19.076 + (hash % 1000) / 10000;
...
export const relationshipService = async (data: CrimeExtractionResult) => {
  try {
    ...
    const transactionResult = await prisma.$transaction(async (tx) => {
      ...
      const reportingStationLoc = locations.find(
        (l) => l.locationType === "REPORTING_STATION",
      );
      if (reportingStationLoc) {
        if (reportingStationLoc.station)
          stationName = reportingStationLoc.station;
        if (reportingStationLoc.district)
          stationDistrict = reportingStationLoc.district;
      } else {
        ...
        if (dbPerson) {
          dbPerson = await tx.person.update({
            ...
            data: {
              ...
              aliases: [...dbPerson.aliases, ...(person.aliases || [])].filter(
      ...
      for (const rel of relationships || []) {
        if (isPersonRelationshipType(rel.type)) {
          ...
          const sourcePerson = createdPersons.find(
            (p) => p.name === rel.source,
          );
          ...
          const targetLocation = createdLocations.find(
            (l) => l.address === rel.target || l.station === rel.target,
          );
