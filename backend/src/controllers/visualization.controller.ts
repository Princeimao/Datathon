...
export const getDistrictStats = async (req: Request, res: Response) => {
    try {
        ...
        const stationStats = stations.map(s => {
            ...
            const typeBreakdown = s.cases.reduce((acc, c) => {
                acc[c.crimeType] = (acc[c.crimeType] || 0) + 1;
                return acc;
            }, {} as Record<string, number>);

            const statusBreakdown = s.cases.reduce((acc, c) => {
        ...
        const districtGroups = stationStats.reduce((acc, stat) => {
            ...
            Object.entries(stat.typeBreakdown).forEach(([type, count]) => {
                acc[district].typeBreakdown[type] = (acc[district].typeBreakdown[type] || 0) + count;
            });
    ...
    try {
        ...
        persons.forEach((p) => {
            nodesMap[p.id] = {
                id: p.id,
                labels: ["Person"],
                properties: { name: p.name, age: p.age, gender: p.gender, riskScore: p.riskScore, aliases: p.aliases }
            };
            ...
            p.phones.forEach((ph) => {
                nodesMap[ph.id] = { id: ph.id, labels: ["Phone"], properties: { number: ph.number } };
                edges.push({ id: `rel-${p.id}-${ph.id}`, source: p.id, target: ph.id, type: "OWNER", properties: {} });
            });

            p.vehicles.forEach((v) => {
                nodesMap[v.id] = { id: v.id, labels: ["Vehicle"], properties: { registrationNo: v.registrationNo, make: v.make, model: v.model } };
                edges.push({ id: `rel-${p.id}-${v.id}`, source: p.id, target: v.id, type: "OWNS_VEHICLE", properties: {} });
            });

            p.addresses.forEach((loc) => {
                nodesMap[loc.id] = { id: loc.id, labels: ["Location"], properties: { address: loc.address, district: loc.district } };
                edges.push({ id: `rel-${p.id}-${loc.id}`, source: p.id, target: loc.id, type: "LIVES_AT", properties: {} });
            });

            p.organization.forEach((om) => {
                nodesMap[om.organization.id] = { id: om.organization.id, labels: ["Organization"], properties: { name: om.organization.name } };
                edges.push({ id: `rel-${p.id}-${om.organization.id}`, source: p.id, target: om.organization.id, type: "MEMBER_OF", properties: { role: om.role } });
            });

            p.cases.forEach((cp) => {
                nodesMap[cp.case.id] = { id: cp.case.id, labels: ["Case"], properties: { caseNumber: cp.case.caseNumber, title: cp.case.title, crimeType: cp.case.crimeType } };
                edges.push({ id: `rel-${p.id}-${cp.case.id}`, source: p.id, target: cp.case.id, type: "INVOLVED_IN", properties: { role: cp.role } });
            });
        });
    ...
    try {
        ...
        const profiles = persons
            .map(p => {
                ...
                const suspectCases = caseInvolvements.filter(c => c.role === "SUSPECT");

                // Calculate a dynamic risk score based on number of cases involved, role, and historical average
                const baseScore = suspectCases.length * 25 + (totalCases - suspectCases.length) * 5;
    ...
    try {
        ...
        for (let i = 0; i < persons.length; i++) {
            for (let j = i + 1; j < persons.length; j++) {
                ...
                // Check shared phones
                const sharedPhones = p1.phones.filter(ph1 => p2.phones.some(ph2 => ph2.id === ph1.id));
                ...
                const sharedVehicles = p1.vehicles.filter(v1 => p2.vehicles.some(v2 => v2.id === v1.id));
                ...
                const sharedLocs = p1.addresses.filter(l1 => p2.addresses.some(l2 => l2.id === l1.id));
                ...
                const sharedOrgs = p1.organization.filter(o1 => p2.organization.some(o2 => o2.organizationId === o1.organizationId));
...
export const getPredictiveStats = async (req: Request, res: Response) => {
    try {
        ...
        const dashboard = districtStats.map((d, index) => {
            ...
            const urbanizationRate = Math.min(98, Math.max(15, Math.abs(hash % 85) + 12));
            const unemploymentRate = parseFloat((3.5 + Math.abs((hash >> 2) % 12) * 0.8).toFixed(1));
            const literacyRate = Math.min(99, Math.max(45, 95 - Math.abs((hash >> 4) % 40)));
            const populationDensity = Math.abs((hash >> 6) % 1500) + 200 + (incidentCount * 25);

            // Compute predictive risk index
            const predictedRiskScore = Math.min(100, Math.round(
                (incidentCount * 2.5) + (unemploymentRate * 3.5) + (urbanizationRate * 0.3) - (literacyRate * 0.2)
            ));

            // Forecast crime rate trends for next 3 months (spatially calculated)
            const forecastTrend = predictedRiskScore > 65 ? "INCREASING" : predictedRiskScore < 40 ? "DECREASING" : "STABLE";
            ...
            return {
                ...
                predictiveAnalysis: {
                    predictedRiskScore,
                    forecastTrend,
                    recommendedPatrolResourceWeight: parseFloat((predictedRiskScore / 10).toFixed(1)),
                    riskLevel: predictedRiskScore > 75 ? "HIGH" : predictedRiskScore > 45 ? "MEDIUM" : "LOW"
                }
            };
        });
...
export const getAnomalies = async (req: Request, res: Response) => {
    try {
        ...
        const anomalies = cases.map(c => {
            ...
            // Pattern checks
            if ((c.crimeType === "HOMICIDE" || c.crimeType === "MURDER") && hour >= 6 && hour <= 18) {
                reasons.push("Homicide occurring during broad daylight");
            }
            if (c.crimeType === "CYBERCRIME" && mo?.weaponType) {
                reasons.push(`Cybercrime involving a physical weapon pattern (${mo.weaponType})`);
            }
            if (hour >= 1 && hour <= 4 && c.crimeType === "FRAUD") {
                reasons.push("Fraud/cyber financial activity peak in midnight (1am - 4am)");
            }
...
export async function calculatePersonSimilarity(targetPersonId: string) {
    ...
    const targetVehicles = target.vehicles.map(v => v.registrationNo).filter(Boolean) as string[];
    const targetLocations = target.addresses.map(l => `${l.latitude.toFixed(4)},${l.longitude.toFixed(4)}`);
    ...
    const matches = others.map(other => {
        ...
        if (target.name && other.name) {
            ...
            if (nameSim > 0.75) {
                ...
                reasons.push(`Similar name: "${other.name}" (confidence: ${Math.round(nameSim * 100)}%)`);
            }
        }
        ...
        if (target.age && other.age) {
            ...
            if (ageDiff <= 3) {
                confidence += 10;
                reasons.push(`Similar age group (age: ${other.age}, diff: ${ageDiff} years)`);
            }
        }

        const aliasOverlap = target.aliases.filter(a => other.aliases.includes(a));
        ...
        const sharedPhones = otherPhones.filter(num => targetPhones.includes(num));
        if (sharedPhones.length > 0) {
            confidence += 50;
            reasons.push(`Shared contact numbers: ${sharedPhones.join(", ")}`);
        }

        const otherVehicles = other.vehicles.map(v => v.registrationNo).filter(Boolean) as string[];
        const sharedVehicles = otherVehicles.filter(reg => targetVehicles.includes(reg));
        if (sharedVehicles.length > 0) {
            confidence += 50;
            reasons.push(`Shared vehicles: ${sharedVehicles.join(", ")}`);
        }

        const otherLocations = other.addresses.map(l => `${l.latitude.toFixed(4)},${l.longitude.toFixed(4)}`);
        const sharedLocations = otherLocations.filter(loc => targetLocations.includes(loc));
        ...
        if (sharedCases.length > 0) {
            confidence += 15;
            reasons.push(`Found in same case investigations: ${sharedCases.length} shared case(s)`);
        }

        let finalConfidence = Math.min(100, confidence);
        if (target.name === other.name && sharedPhones.length > 0 && finalConfidence > 90) {
            finalConfidence = 100;
        }
...
export const getTimelineStats = async (req: Request, res: Response) => {
    try {
        ...
        cases.forEach(c => {
            ...
            bucket.caseCount++;
            bucket.crimeTypes[c.crimeType] = (bucket.crimeTypes[c.crimeType] || 0) + 1;
...
export const getCaseBoard = async (req: Request, res: Response) => {
    try {
        ...
        // Helper to push node
        const addNode = (id: string, type: string, label: string, properties: any) => {
            if (!id) return false;
            if (!addedNodeIds[id]) {
        ...
        caseObj.organizations.forEach((co: any) => {
            ...
            org.members.forEach((member: any) => {
                ...
                addEdge(member.personId, org.id, `MEMBER_OF_${member.role || "MEMBER"}`);
            });
        });
...
export const getDashboardSummary = async (req: Request, res: Response) => {
    try {
        ...
        cases.forEach(c => {
            crimeTypeBreakdown[c.crimeType] = (crimeTypeBreakdown[c.crimeType] || 0) + 1;
        });

        const crimeMix = Object.entries(crimeTypeBreakdown).map(([name, value]) => ({
            name,
            value
        }));

        // 3. District-level stats (for bar chart)
        const districtMap: Record<string, { district: string; state: string; cases: number; crimeTypes: Record<string, number> }> = {};
        cases.forEach(c => {
            ...
            districtMap[district].crimeTypes[c.crimeType] = (districtMap[district].crimeTypes[c.crimeType] || 0) + 1;
        });
        ...
        cases.forEach(c => {
            const hour = new Date(c.incidentDate).getHours();
            if ((c.crimeType === "HOMICIDE" || c.crimeType === "MURDER") && hour >= 6 && hour <= 18) anomalyCount++;
            if (c.crimeType === "CYBERCRIME" && c.modusOperandi?.weaponType) anomalyCount++;
            if (hour >= 1 && hour <= 4 && c.crimeType === "FRAUD") anomalyCount++;
        });

        // 5. MO patterns (for radar chart and network cards)
        const moMap: Record<string, { mo: string; suspects: Set<string>; incidents: number; confidence: number }> = {};
        for (const c of cases) {
            if (c.modusOperandi) {
                const moName = c.modusOperandi.name;
                if (!moMap[moName]) {
                    moMap[moName] = { mo: moName, suspects: new Set(), incidents: 0, confidence: 0 };
                }
        ...
        const moIds = [...new Set(cases.filter(c => c.modusOperandiId).map(c => c.modusOperandiId!))];
        if (moIds.length > 0) {
        ...
        const moNetwork = Object.values(moMap).map(m => ({
            mo: m.mo,
            suspects: m.suspects.size,
            incidents: m.incidents,
            confidence: m.incidents > 0 ? Math.min(99, Math.round(60 + m.incidents * 3 + m.suspects.size * 2)) : 0
        })).sort((a, b) => b.confidence - a.confidence);
        ...
        const predictiveRisks = districtStats.map(d => {
            ...
            const unemployment = parseFloat((3.5 + Math.abs((hash >> 2) % 12) * 0.8).toFixed(1));
            const forecast = Math.min(100, Math.round(
        ...
        const timelineMap: Record<string, { label: string; cases: number; crimeTypes: Record<string, number> }> = {};
        cases.forEach(c => {
            const date = new Date(c.incidentDate);
            let label = "";
            if (groupBy === "day") {
            ...
            timelineMap[label].crimeTypes[c.crimeType] = (timelineMap[label].crimeTypes[c.crimeType] || 0) + 1;
        });

        const timeline = Object.values(timelineMap).sort((a, b) => a.label.localeCompare(b.label)).map(t => ({
            label: t.label,
            cases: t.cases,
            forecast: Math.round(t.cases * 1.12),
            sentiment: Math.max(-82, Math.min(36, -28 + Math.round(Math.random() * 20 - 10))),
            ...t.crimeTypes
        }));

        // 8. High risk zone count
        const highRiskCount = predictiveRisks.filter(d => d.risk === "high").length;
...
export const getMapData = async (req: Request, res: Response) => {
    try {
        ...
        const stateGroups: Record<string, { state: string; totalCases: number; crimeTypes: Record<string, number>; districts: Record<string, number> }> = {};
        points.forEach(p => {
            const s = p.state || "Unknown";
            if (!stateGroups[s]) {
                stateGroups[s] = { state: s, totalCases: 0, crimeTypes: {}, districts: {} };
            }
            stateGroups[s].totalCases++;
            stateGroups[s].crimeTypes[p.crimeType] = (stateGroups[s].crimeTypes[p.crimeType] || 0) + 1;
        ...
        if (riskLevel && riskLevel !== "All Risk Levels") {
            filteredStateStats = stateStats.filter(s => s.riskLevel === riskLevel.toLowerCase());
        }
        ...
        res.status(200).json({
            ...
            data: {
                ...
                summary: {
                    ...
                    highRiskStates: stateStats.filter(s => s.riskLevel === "high").length,
